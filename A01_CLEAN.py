...
csv_database_A = create_engine('sqlite:////home/www_adm/ssd/CHIP_OUTPUT/STEP_01.db')       
dfa=['XXX',20,1,1,1,1,1]
dfa=pd.DataFrame(dfa)
dfa=dfa.transpose()
dfa.columns=['seq','L','sim_level','gene','Tm','Gc','dG']
...
#            
        ...
        pool = ThreadPool(multiprocessing.cpu_count())
        V1A=pool.starmap(acho,zip(VOC_LIST,itertools.repeat(GENE_SEQ)))
        pool.close()
        pool.join()          
        ...
        RESPONSE=RESPONSE[(RESPONSE['sim_level']<EXON_CMP) | (RESPONSE['sim_level']==1)]
        #RESPONSE=RESPONSE[RESPONSE['sim_level']<EXON_CMP]
        
        RESPONSE['gene']=MRNA_BLOCK[2].iloc[0]
        RESPONSE=RESPONSE[['seq','L','sim_level','gene','Tm','Gc','dG']]
